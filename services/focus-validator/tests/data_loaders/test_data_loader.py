...
class TestDataLoader(unittest.TestCase):
    ...
    def test_load_csv_success(self):
        ...
        self.assertEqual(result["col1"][0], 1)
    ...
    def test_load_parquet_success(self, mock_parquet_load):
        ...
        mock_df = pl.DataFrame({"col1": [1, 3], "col2": [2, 4]})
    ...
    def test_resilient_loading_integration(self):
        ...
        try:
            ...
            self.assertEqual(result['NumericCol'].dtype, pl.Float64)
            # DateCol should be dropped due to invalid date
            self.assertNotIn('DateCol', result.columns) 
            self.assertIn('DateCol', csv_loader.failed_columns)
            self.assertEqual(result['StringCol'].dtype, pl.Utf8)
            
            # Check that invalid values were coerced to null
            self.assertTrue(result['NumericCol'][1] is None)  # INVALID -> null
            # Note: DateCol column may be dropped due to conversion issues, check if it exists
            if 'DateCol' in result.columns:
                self.assertTrue(result['DateCol'][1] is None)     # BAD_DATE -> null
...
class TestDataLoaderEdgeCases(unittest.TestCase):
    ...
    def test_empty_csv_file(self):
        ...
        try:
            ...
            result = loader.load()
            # Test should handle whatever polars does with empty files
    ...
    def test_malformed_csv_file(self):
        ...
        try:
            loader = DataLoader(temp_filename)
            # polars might handle this gracefully or raise exception
            result = loader.load()
            if result is not None:
                self.assertIsInstance(result, pl.DataFrame)
        except Exception as e:
