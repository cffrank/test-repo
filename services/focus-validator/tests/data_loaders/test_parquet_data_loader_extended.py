...
class TestParquetDataLoaderBasic(unittest.TestCase):
    ...
    @patch('focus_validator.data_loaders.parquet_data_loader.pl.read_parquet')
    ...
    @patch('focus_validator.data_loaders.parquet_data_loader.pl.read_parquet')
    def test_polars_exception_handling(self, mock_read_parquet):
        """Test handling when pl.read_parquet raises unexpected exceptions."""
        mock_read_parquet.side_effect = Exception("Unexpected polars error")
    ...
    def test_successful_mocked_load(self, mock_read_parquet):
        """Test successful parquet loading with mocked polars."""
    ...
    def test_load_returns_none_handling(self, mock_read_parquet):
        """Test handling when polars returns None (unusual but possible)."""
...
class TestParquetDataLoaderResilientLoading(unittest.TestCase):
    ...
    def test_successful_load_with_type_conversion(self):
        ...
        self.assertIsInstance(result, pl.DataFrame)
        self.assertEqual(len(result), 3)
        self.assertEqual(result['Amount'].dtype, pl.Float64)
        self.assertEqual(result['Date'].dtype, pl.Datetime('us', 'UTC'))
        self.assertEqual(result['Zone'].dtype, pl.Utf8)

    def test_type_conversion_with_invalid_data(self):
        """Test type conversion resilience - columns with invalid dates should be dropped."""
        ...
        # Should succeed with some columns dropped
        self.assertIsInstance(result, pl.DataFrame)
        self.assertEqual(len(result), 3)
        
        # Check numeric coercion
        self.assertEqual(result['NumericCol'][0], 123.45)
        self.assertTrue((result['NumericCol'][1] is None))  # Invalid -> NaN
        self.assertEqual(result['NumericCol'][2], 67.89)
        
        # DateCol should be dropped due to invalid date (BAD_DATE)
        self.assertNotIn('DateCol', result.columns)
        
        # Check string conversion
        self.assertIn('StringCol', result.columns)
        self.assertEqual(result['StringCol'].dtype, pl.Utf8)
